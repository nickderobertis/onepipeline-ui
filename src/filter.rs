...
//! `exclude` winning and an absent `include` admitting everything. The three
//! each declare it over their own vocabulary, and `onepipeline` — the one crate
//! that names every producer — owns the stack's reading of it
//! (`onepipeline::filter::{EventFilter, Matcher}`, published as
//! `agent.event-filter@1` in its `schemas/events.json`). This crate keeps its
...
//! holds the copy to the engine's own matcher document rather than to a second
//! reading of the wire.
//!
...
impl EventFilter {
    ...
    /// engine's own declaration — so the conversion is the wire: serialized by this
