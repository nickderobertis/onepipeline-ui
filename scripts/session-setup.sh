...
verify_prereqs() {
  local tool
  for tool in uv rustup cargo node; do
    command -v "$tool" >/dev/null 2>&1 \
      || log "$tool not on PATH (install rustup: https://rustup.rs/ ; uv: https://docs.astral.sh/uv/ ; node: https://nodejs.org/)"
  done
